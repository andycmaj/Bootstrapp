...
TheChunnel.FrontEnd's UI src lives in **src/FrontEnd/admin-ui/src**. It interacts with the FrontEnd API running
in the same domain:port.
For development on the FrontEnd UI, follow the instructions below.

## Dev Mode

In Dev mode, changes to javascript auto-reload in the browse.

### Requirements

1. nodejs / npm
2. Install `npm` dependencies using `yarn install` from **TheChunnel.FrontEnd/admin-ui** project root.
3. Following Environment variables must be set.

```bash
# in your dotnet terminal
$ export ASPNETCORE_ENVIRONMENT=Development
$ export AWS_ACCESS_KEY_ID=<ACCESS_KEY>
$ export AWS_SECRET_ACCESS_KEY=<SECRET_KEY>

# in your yarn/npm terminal
$ export REACT_APP_CHUNNEL__APIKEY=sKP7eeSDHICSEnb4B8Skkrgdbb9oNUwS
```

### Steps

1. run FrontEnd API. `dotnet restore && dotnet run` in **src/TheChunnel.FrontEnd**.
2. In another terminal, set the following environment variable: `export REACT_APP_CHUNNEL__APIKEY=sKP7eeSDHICSEnb4B8Skkrgdbb9oNUwS` then run dev server using `npm start` in **src/TheChunnel.FrontEnd/admin-ui**. This will open a browser window to the development ui.

## Release Mode

Auth0 auth is enabled in release mode.
Changes to javascript do not take effect until you re-run **step 1** and restart the FrontEnd.

### Requirements

See Dev-Mode requirements.

### Steps

1. run `./build.sh -t BuildUi` from the root of TheChunnel's repo.
2. run FrontEnd. `dotnet restore && ./run.sh` in **src/TheChunnel.FrontEnd**.
3. browse to **http://localhost:5000**. In release mode, UI and API are on same port.

## Troubleshooting

#### Missing Microsoft.CSharp.dll when loading MVC views

```bash
fail: Microsoft.AspNetCore.Server.Kestrel[13]
      => RequestId:0HKU05BMPLOCT RequestPath:/home
      Connection id "0HKU05BMLCR8V": An unhandled exception was thrown by the application.
System.InvalidOperationException: Can not find reference assembly 'Microsoft.CSharp.dll' file for package Microsoft.CSharp
  at Microsoft.Extensions.DependencyModel.Resolution.ReferenceAssemblyPathResolver.TryResolveAssemblyPaths (Microsoft.Extensions.DependencyModel.CompilationLibrary library, System.Collections.Generic.List`1 assemblies) <0x112462940 + 0x0012b> in <filename unknown>:0
...
```
Workaround:
```bash
/Library/Frameworks/Mono.framework/Versions/4.4.1/lib/mono/xbuild-frameworks $ sudo cp ../4.5/*.dll .
/Library/Frameworks/Mono.framework/Versions/4.4.1/lib/mono/xbuild-frameworks $ sudo cp ../4.5/Facades/*.dll .
```
